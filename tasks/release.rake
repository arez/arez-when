...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/arez/arez-when/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']})
[Full Changelog](https://github.com/arez/arez-when/compare/v#{ENV['PREVIOUS_PRODUCT_VERSION']}...v#{ENV['PRODUCT_VERSION']})
HEADER
...
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('arez/arez-when', tag, :name => tag, :body => changes, :draft => false, :prerelease => true)

      candidates = client.list_milestones('arez/arez-when').select {|m| m[:title].to_s == tag}
      unless candidates.empty?
        milestone = candidates[0]
        unless milestone[:state] == 'closed'
          client.update_milestone('arez/arez-when', milestone[:number], :state => 'closed')
        end
      end
    end
  end
